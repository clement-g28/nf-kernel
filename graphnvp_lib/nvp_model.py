...
from graphnvp_lib.graph_nvp.hyperparams import Hyperparameters
from graphnvp_lib.graph_nvp.coupling import AffineNodeFeatureCoupling, AffineAdjCoupling, \
    AdditiveNodeFeatureCoupling, AdditiveAdjCoupling


class GraphNvpModel(nn.Module):
    def __init__(self, hyperparams: Hyperparameters, device):
        ...
        # self.prior_ln_var = torch.tensor([0.7],device=self.device, dtype=torch.float32)
        self.prior_ln_var = nn.Parameter(torch.zeros([1]))
        nn.init.constant_(self.prior_ln_var, 1e-5)
        self.constant_pi = torch.tensor([3.1415926535], device=self.device, dtype=torch.float32)
        ...
        for i in range(self.num_coupling['channel']):
            transforms += [channel_coupling(self.num_nodes, self.num_relations, self.num_features,
                                            self.masks['channel'][i % self.num_masks['channel']], self.device,
                                            hyperparams.num_gcn_layer,
                                            num_masked_cols=int(self.num_nodes / self.num_masks['channel']),
                                            ch_list=self.gnn_channels,
                                            batch_norm=self.apply_batch_norm, use_switch=hyperparams.use_switch)]
        for i in range(self.num_coupling['node']):
            transforms += [node_coupling(self.num_nodes, self.num_relations, self.num_features,
                                         self.masks['node'][i % self.num_masks['node']], self.device,
                                         num_masked_cols=int(self.num_nodes / self.num_masks['channel']),
    ...
    def forward(self, adj, x):
        h = x.clone()
        if self.training:
            h = h + torch.rand_like(x) * 0.9  # noise_scale default 0.9
        sum_log_det_jacs_x = torch.zeros(h.shape[0], device=self.device, requires_grad=True)
        sum_log_det_jacs_adj = torch.zeros(h.shape[0], device=self.device, requires_grad=True)
        ...
        # adj = adj + 0.9 * torch.rand(adj.shape, device=self.device)

        if self.training:
            adj = adj + torch.rand_like(adj) * 0.9  # (256,4,9,9) noise_scale default 0.9
    ...
    def _init_params(self, hyperparams):
        ...
        self.apply_batch_norm = hyperparams.apply_batch_norm
        self.additive_transformations = hyperparams.additive_transformations
