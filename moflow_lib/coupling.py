...
class AffineCoupling(nn.Module):  # delete
    def __init__(self, in_channel, hidden_channels, affine=True,
                 mask_swap=False):  # filter_size=512,  --> hidden_channels =(512, 512)
        super(AffineCoupling, self).__init__()
        ...
        for h in vh:
            self.layers.append(nn.Conv2d(last_h, h, kernel_size=3, padding=1))
            self.norms.append(nn.BatchNorm2d(
                h))  # , momentum=0.9 may change norm later, where to use norm? for the residual? or the sum
            # self.norms.append(ActNorm(in_channel=h, logdet=False)) # similar but not good
            last_h = h
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            # # log_s, t = self.net(in_a).chunk(2, 1)  # (2,12,32,32) --> (2,6,32,32), (2,6,32,32)
            # log_s, t = self._s_t_function(in_a)
            # s = torch.exp(log_s)
            # out_b = (in_b + t) * s  # different affine bias , no difference to the log-det # (2,6,32,32) More stable, less error
            # # out_b = in_b * s + t
...
class GraphAffineCoupling(nn.Module):
    def __init__(self, in_adj_dim, n_node, in_dim, hidden_dim_dict, masked_row, affine=True):
        ...
        for out_dim in self.hidden_dim_gnn:  # What if use only one gnn???
            self.net.append(GraphConv(last_dim, out_dim, in_adj_dim))
    ...
    def forward(self, adj, input):
        ...
        # masked_x = self.mask * input
        # log_s, t = self._s_t_function(adj, masked_x)  # s must not equal to 0!!!
        # s = torch.exp(log_s)
        #
        # if self.affine:
        #     out = masked_x + (1 - self.mask) * (input + t) * s
        #     # out = masked_x + (1-self.mask) * (input * s + t)
        #     logdet = torch.sum(log_s.view(input.shape[0], -1), 1)  # possibly wrong answer
        # else:  # add coupling
        #     out = masked_x + t * (1 - self.mask)
        #     logdet = None
        # return out, logdet
