...
def gaussian_nll(x, mean, ln_var, reduce='sum'):
    ...
    if reduce == 'sum':
        ...
        return loss
# -0.5 * u.pow(2) - 0.5 * math.log(2 * math.pi)
...
class MoFlow(nn.Module):
    def __init__(self, hyper_params: Hyperparameters):
        ...
        self.atom_model = GlowOnGraph(
            in_adj_dim=hyper_params.b_n_type,
    ...
    def log_prob(self, z, logdet, return_probs=False):  # z:[(256,45), (256,324)] logdet:[(256,),(256,)]
        ...
        adj_nlogprob = gaussian_nll(z[1], torch.zeros(self.b_size).to(z[0]), ln_var_adj, reduce='no')
        nll_adj = torch.mean(torch.sum(adj_nlogprob, dim=1) - logdet[1])
        nll_adj = nll_adj / (self.b_size * math.log(2.))  # the negative log likelihood per dim with log base 2

        x_nlogprob = gaussian_nll(z[0], torch.zeros(self.a_size).to(z[0]), ln_var_x, reduce='no')
        nll_x = torch.mean(torch.sum(x_nlogprob, dim=1) - logdet[0])
        nll_x = nll_x / (self.a_size * math.log(2.))  # the negative log likelihood per dim with log base 2
        if nll_x.item() < 0:
            print('nll_x:{}'.format(nll_x.item()))
        res = [nll_x, nll_adj]
        if return_probs:
            res += [x_nlogprob.mean().item(), adj_nlogprob.mean().item()]
        return res

    def save_hyperparams(self, path):
        self.hyper_params.save(path)

    # def log_prob_bis(self, dist_x, dist_adj, logdet, dweight=1):
    def log_prob_bis(self, dist, logdet, dweight=1):
        logdet[0] = logdet[0] - self.a_size * math.log(2.)  # n_bins = 2**n_bit = 2**1=2
        logdet[1] = logdet[1] - self.b_size * math.log(2.)

        # dist_adj = func(z_adj, dist_rows, gram_idx, pool='mean')
        # dist_x = func(z_x, dist_rows, gram_idx, pool='mean')

        # distance to probability
        # prob_adj = -torch.log(1 / (1 + dist_adj))
        prob = -torch.log(1 / (1 + dist))
        nll_adj = dweight * self.b_size * prob - torch.mean(logdet[1])
        nll_adj = nll_adj / (self.b_size * math.log(2.))  # the negative log likelihood per dim with log base 2

        # prob_x = -torch.log(1 / (1 + dist_x))
        nll_x = dweight * self.a_size * prob - torch.mean(logdet[0])
        nll_x = nll_x / (self.a_size * math.log(2.))  # the negative log likelihood per dim with log base 2
        if nll_x.item() < 0:
            print('nll_x:{}'.format(nll_x.item()))
    ...
    def log_prob_sgw(self, logdet, sgw_x, sgw_adj, dweight=1):
        logdet[0] = logdet[0] - self.a_size * math.log(2.)  # n_bins = 2**n_bit = 2**1=2
        logdet[1] = logdet[1] - self.b_size * math.log(2.)

        # distance to probability
        prob_adj = -torch.log(1 / (1 + sgw_adj))
        nll_adj = dweight * self.b_size * prob_adj - torch.mean(logdet[1])
        nll_adj = nll_adj / (self.b_size * math.log(2.))  # the negative log likelihood per dim with log base 2

        prob_x = -torch.log(1 / (1 + sgw_x))
        nll_x = dweight * self.a_size * prob_x - torch.mean(logdet[0])
        nll_x = nll_x / (self.a_size * math.log(2.))  # the negative log likelihood per dim with log base 2
        if nll_x.item() < 0:
            print('nll_x:{}'.format(nll_x.item()))
