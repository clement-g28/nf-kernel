...
def plt_flow_density(prior_logdensity, inverse_transform, ax, npts=100, memory=100, title="$q(x)$", device="cpu",
                     shape=2):
    # npt = math.floor(math.exp(math.log(npts)/shape))
    side = np.linspace(LOW, HIGH, npts)
    # side = np.linspace(LOW, HIGH, npt)
    # l = [side for _ in range(shape)]
    # xx_tup = list(np.meshgrid(l))
    # xx_tup = [xx.reshape(-1, 1) for xx in xx_tup]
    # x = np.hstack(xx_tup)
    xx, yy = np.meshgrid(side, side)
    x = np.hstack([xx.reshape(-1, 1), yy.reshape(-1, 1)])
    if x.shape[1] < shape:
        margin = np.zeros((x.shape[0], shape - x.shape[1]))
        x = np.concatenate((x, margin), axis=1)
...
def plt_flow_samples(prior_sample, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu", n_dim=2):
    z = prior_sample(npts * npts, n_dim).type(torch.float32).to(device)
    zk = []
...
def visualize_transform(
        ...
        npts=100, memory=100, device="cpu"
):
