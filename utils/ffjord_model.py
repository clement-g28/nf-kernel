...
import ffjord_lib.layers.wrappers.cnf_regularization as reg_lib
import ffjord_lib.spectral_norm as spectral_norm
import ffjord_lib.layers as layers
from ffjord_lib.layers.odefunc import divergence_bf, divergence_approx
...
def build_model_tabular(args, dims, regularization_fns=None):
    ...
    chain = [build_cnf() for _ in range(args.n_block)]
    if args.batch_norm:
        bn_layers = [layers.MovingBatchNorm1d(dims, bn_lag=args.bn_lag) for _ in range(args.n_block)]
